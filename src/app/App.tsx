...
export default function App() {
  ...
  const deriveSandboxReplyGateState = useCallback((): SandboxReplyGateState => {
    ...
    const sandboxFlow = sandboxModeRef.current.getState().sandboxFlow;
    const replyGateType = sandboxFlow.replyGateActive ? 'consonant_wait_reply' : null;
    const replySourceMessageId = qnaStateRef.current.active.questionMessageId;
    const replyTarget = sandboxFlow.replyTarget;
    const replyGateArmed = sandboxFlow.replyGateActive;
    ...
    const timer = window.setInterval(() => {
      ...
      if (modeRef.current.id === 'sandbox_story' && sandboxState.flow.step === 'WAIT_REPLY_3' && sandboxReplyToActive) {
        ...
        if (now - sandboxTechBacklogLastAtRef.current >= 30_000) {
          ...
          const backlog = [...sandboxModeRef.current.getState().sandboxFlow.backlogTechMessages, '技術故障：訊號不穩，暫時卡住', '技術故障：系統延遲，先別重整'].slice(-8);
          sandboxModeRef.current.setSandboxFlow({ backlogTechMessages: backlog });
  ...
  const sandboxFreezeAndWaitForReply = useCallback((askedAt: number, reason: string, waitStep: 'WAIT_REPLY_1' | 'WAIT_REPLY_2' | 'WAIT_REPLY_3') => {
    ...
    sandboxModeRef.current.setSandboxFlow({ replyGateActive: true, replyTarget: lockStateRef.current.target });
  ...
  useEffect(() => {
    ...
    if (sandboxState.flow.step === 'FLUSH_TECH_BACKLOG') {
      ...
      sandboxModeRef.current.setSandboxFlow({ backlogTechMessages: [] });
      sandboxTechBacklogLastAtRef.current = 0;
      sandboxTechBacklogTotalWaitMsRef.current = 0;
