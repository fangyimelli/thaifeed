...
export const createSandbox360InitialState = () => {
  ...
  hint: { active: false, lastText: '', count: 0, lastShownAt: 0, source: '', requested: false, emitter: '', generatedText: '' },
  ...
  pinnedReply: null as SandboxPinnedReplyState | null,
  ...
  viewer: { currentShot: 'center', targetShot: 'center', currentPosX: 50, targetPosX: 50, posY: 50, time: 0, scale: 1.05, leftPosX: 18, centerPosX: 50, rightPosX: 82, lastCommandAt: 0 }
  };
};

export function ensureSandbox360StateShape(raw: any) {
  const base = createSandbox360InitialState();
  const next = { ...base, ...(raw ?? {}) };
  ...
  next.pinnedReply = raw?.pinnedReply ? { ...raw.pinnedReply } : null;
...
export function createSandbox360Mode(): GameMode & Record<string, any> {
  let ssot: NightScript = NIGHT1;
  let state: any = createSandbox360InitialState();
  ...
  const bootstrapRuntime = (reason = 'mode_entry', at = Date.now(), minDurationMs = SANDBOX_BOOTSTRAP_MIN_DURATION_MS) => {
      ...
      state.reply = { ...createSandbox360InitialState().reply };
      state.pinnedReply = null;
      state.consonantJudgeAudit = { ...createSandbox360InitialState().consonantJudgeAudit };
  ...
  return {
    ...
    setPinnedReply: (entry: SandboxPinnedReplyState | null) => {
      state.pinnedReply = entry ? { ...entry } : null;
    },
