...
import { getChatLintReason } from '../../chat/ChatLint';
...
const PERSONA_PROFILES: Record<PersonaId, PersonaProfile> = {
  chill: makeProfile('chill', ['我先慢慢看', '先別急我還在盯', '這裡有點悶', '等下再回放一次']),
  nervous: makeProfile('nervous', ['欸我手心在冒汗', '不要突然衝出來', '我真的怕', '我先吸一口氣']),
  troll: makeProfile('troll', ['這鏡頭很會挑時間', '我看你很懂節奏', '彈幕等下要失控', '好啦你贏了']),
  quiet: makeProfile('quiet', ['嗯我有看到', '我先記住這裡', '不太舒服', '再看一次']),
  observer: makeProfile('observer', ['亮度剛剛掉了一拍', '邊緣像被拉了一下', '前後看起來有落差', '這裡值得重播']),
  hype: makeProfile('hype', ['這波直接拉滿', '節奏衝起來了', '我整個醒了', '不要停這裡']),
  skeptical: makeProfile('skeptical', ['先別急著定論', '這會不會是反光', '我想再看一輪', '先看前後差別']),
  empath: makeProfile('empath', ['你先穩住呼吸', '不要硬撐我在', '看不下去就休息', '我陪你慢慢看']),
  meme: makeProfile('meme', ['這裡像梗圖現場', '我腦袋已經配音了', '聊天室要開始玩梗了', '先存這個反應']),
  foodie: makeProfile('foodie', ['我剛拿零食差點掉', '這感覺比辣還猛', '我杯子都握緊了', '胃一縮']),
  gamer: makeProfile('gamer', ['這像Boss前搖', '警戒條直接亮紅', '我直覺要翻車', '這鏡頭像伏筆']),
  sleepy: makeProfile('sleepy', ['我本來快睡著', '現在完全醒了', '這一下把我拉回來', '眼睛張開了']),
  detective: makeProfile('detective', ['我先記這個位置', '這裡像有人經過', '線索在暗部', '可以做標記']),
  caretaker: makeProfile('caretaker', ['先喝口水再看', '太緊就先停', '別逞強喔', '大家慢慢來']),
  chaotic: makeProfile('chaotic', ['我腦內警報亂叫', '這裡太邪門了', '不行我先大叫', '氣氛炸開']),
  polite: makeProfile('polite', ['借過我補一句', '這裡讓人發涼', '請大家注意一下', '失禮了但有點怪']),
  impatient: makeProfile('impatient', ['快點回放剛剛那下', '不要拖我想看重點', '感覺快來了', '先講結論很怪']),
  storyteller: makeProfile('storyteller', ['這感覺像老屋在吐氣', '畫面像有人貼著牆走', '節奏像在等人回頭', '這裡很有故事感']),
...
export function generateChatMessageV2(input: GenerateInput): ChatMessage {
  ...
  for (let i = 0; i < 48; i += 1) {
    ...
    if (recentViewport.includes(normalized)) continue;
