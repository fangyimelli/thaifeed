# Classic Message Review Packet

This document is generated for classic **source/category review**. It is a review packet only and is **not** an import source.

## Purpose split

- `docs/classic-message-review.md`: source/category-first review for auditors who want to inspect content pools by category.
- `docs/classic-flow-message-review.md`: flow/player-experience-first review for auditors who want to inspect what the player sees step by step.
- Classic remains review-first; neither packet creates an editable import path.
...
- sourceOfTruth: registry
- ownership: shared_tooling / ownerMode: classic
...
### classic.pool.ui_status

- status: active
- sourceFile: src/chat/ChatPools.ts
- sourceSymbol: TYPE_FALLBACK_POOLS.UI_STATUS
- sourceOfTruth: registry
- ownership: unset / ownerMode: classic
- currentVariants: 系統已就緒
- tokens: none
- runtimeWrapper: none
- reviewSlot: pending

### classic.system.ui_status

- status: active
- sourceFile: src/chat/ChatPools.ts
- sourceSymbol: SYSTEM_POOLS.ui_status
- sourceOfTruth: registry
- ownership: unset / ownerMode: classic
- currentVariants: 聲音已啟用, 畫面已準備完成, 初始化完成, 部分非必要素材載入失敗 遊戲可正常進行
- tokens: none
- runtimeWrapper: none
- reviewSlot: pending
