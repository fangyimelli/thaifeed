...
- `sandbox_360_test` 僅作為 sandbox 專用視角指令測試模式：聊天室命中方向指令時，只更新 viewer state，並由 `SceneView` 讀取 yaw/pitch 映射成 viewport 內 oversized scene 的平移裁切，不接管 classic 或 sandbox_story 的 reply pipeline。
