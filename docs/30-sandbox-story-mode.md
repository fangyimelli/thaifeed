...
> 2026-03-05 更新：sandbox 主流程 SSOT 已改為 `flow.questionIndex + flow.step`；phase 僅保留渲染/路由表象。

固定 step 鏈：

0 `PREHEAT`（30s，不出題）
1 `ASK_CONSONANT`
2 `WAIT_ANSWER`（15s 未回硬停聊天 + 等你回覆）
3 `ANSWER_GLITCH_FLOOD`
4 `REVEAL_WORD`
5 `WORD_RIOT`
6 `VIP_TRANSLATE`
7 `MEANING_GUESS`
8 `ASK_PLAYER_MEANING`
9 `ADVANCE_NEXT`
