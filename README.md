...
- 2026-03-25：移除 `sandbox_360_test` 舊 TV anchor 估算框（`{ x: 2240, y: 1154, w: 418, h: 244 }`），改為使用者紅框校正後的單一 SSOT 常數，避免估算/人工雙軌並存。
...
- [sandbox][integration-fix][reveal-transition-commit-ssot] 修正 authoritative transition commit 脫鉤：當 `REVEAL_WORD` 已滿足 `phase=done/rendered=true/timing ready/transitionEligible=true`，立即以同一個 commit 時間戳執行 `setFlowStep('POST_REVEAL_CHAT')`，並同步落盤 `reveal.transitionCommitAttempted/CommittedAt/CommitReason/CommitBlockedBy`；`cleanup_hidden` 不再作為 reveal completion 轉場阻擋來源。`POST_REVEAL_CHAT` 與 `ADVANCE_NEXT` 仍維持單一路徑推進並保證可觀測 `postReveal.enteredAt`、`advanceNext.enteredAt`、`nextQuestion.ready/emitted`、`flow.questionIndex/currentPrompt` 切題一致。
- [sandbox][integration-fix][reveal-transition-ssot-unblock] 修正 `REVEAL_WORD -> POST_REVEAL_CHAT` 卡死：當 `reveal.phase=done && rendered=true && guardReady/completionReady` 成立即強制推進，不再被 `visible=false(cleanup_hidden)` 阻擋；`cleanup_hidden` 降級為 warning。補齊 bounded recovery（REVEAL 完成後逾時自動轉 POST_REVEAL_CHAT）、`postReveal.enteredAt/advanceNext.enteredAt` 與 `reveal.transitionEligible/transitionBlockedBy` 可觀測性，並維持 `POST_REVEAL_CHAT -> ADVANCE_NEXT -> nextQuestion emit` 單一路徑。
- [sandbox][integration-fix][debug-action-authority-reconciliation] 重構 Pass Flow / Force Correct Now / Force Next Question 的 authoritative 邊界：Pass Flow 僅走合法 stage 前進；Force Correct Now 僅使用 currentPrompt.questionId 對應 canonical consonant（含 acceptedCandidates 落盤）並走 `ANSWER_EVAL -> REVEAL_WORD`；Force Next Question 僅切到下一題可回答狀態（重建 currentPrompt/replyGate/renderSync，清空 reveal/parse/judge/consume 殘留），禁止直接 reveal。另補 debugAction observability（name/intent/source/target/resultStep/usedCanonicalAnswer/usedAcceptedCandidates/reconciled）、scene key canonicalization（`oldhouse_room_loopN` -> `loopN`）、renderedQuestionId bounded fallback、與 REVEAL done+rendered+timing 即可進 POST_REVEAL（`visible=false` 僅視覺警示）。
- [sandbox][integration-fix][authoritative-reply-consume-ssot] 修正 `Night Smoke Test`/UI 送出/模擬送出共用同一 authoritative consume 管線：訊息注入後會落 `reply.lastInjected*`，consume 成功會落 `reply.lastConsumed*`，並以 `reply.consumeResult/consumeBlockedReason` 明確區分「已顯示但未 consume」原因；`WAIT_REPLY_1 -> ANSWER_EVAL -> REVEAL_WORD -> POST_REVEAL_CHAT -> ADVANCE_NEXT` 全程以 flow step + stage-scoped blocked reason 驅動，不再讓 stale `reveal_guard_blocked:*` 污染 reply 階段。
- [sandbox][integration-fix][reveal-postreveal-advance-guard-hardening] 修正 Q1 正解後可能卡在 `REVEAL_WORD`：當 `reveal.phase=done && rendered=true` 但 timing 欄位缺失時，會先自動補齊 `startedAt/finishedAt` 再以 `reveal_word_done_timing_repaired` 推進 `POST_REVEAL_CHAT`；`POST_REVEAL_CHAT -> ADVANCE_NEXT`/`ADVANCE_NEXT -> emit` 改用 stage-scoped blocked reason（`reveal_guard_blocked:*` / `post_reveal_blocked:*` / `advance_next_blocked:*` / `emitted`），並新增 `nextQuestion.stage`、`nextQuestion.blockedReason.source`、`reveal.guardReady`、`reveal.hasObservableTiming`、`postReveal.guardReady`、`advanceNext.guardReady` 以直觀定位停點；`scene_not_synced` 維持 warning，不阻斷第二題 emit。
- [sandbox][integration-fix][reveal-cleanup-video-recovery-smoke-judge-audit] 修正 REVEAL_WORD 後 prompt glyph 清理與泡泡關閉（含 debug source=`reveal_prompt_cleanup`），reveal lifecycle 新增 `cleanupAt` 並避免覆寫 `finishedAt`；videoA/videoB onError 不再直接熄滅整個 video assets、保留 swap 診斷並讓 render sync 可恢復；submit 成功保留 authoritative `messageId`，Night Smoke Test 不再誤報 `message_injected_but_not_consumed`；consume 成功後 judge audit 持續完整落盤。
- [sandbox][integration-fix][videoA-render-recovery-and-smoke-consume-audit] 修正 sandbox 影片/渲染不同步與 Night Smoke Test 假失敗：補齊 videoA/videoB 載入錯誤診斷（source/readyState/errorCode/active slot/last swap result）、失敗回退 swap 結果、render sync 在 `replyGate(consonant_answer+armed+canReply)` 時強制 `scene_not_synced_warning` 可見恢復，避免 `stateQuestionId` 長期卡住；同時讓 `submitChat` 回傳真實 `messageId`，Night Smoke Test 以 `lastReplyEval.messageId` 驗證 auto-answer 走 authoritative consume path，並保留完整 judge audit（expectedConsonant/acceptedCandidates/compareInput/compareMode/result/resultReason/gate/source ids/consumedAt）。
- [sandbox][audit][reveal-pipeline-observability] 針對 `REVEAL_WORD`→`POST_REVEAL_CHAT` 可見性審計，補齊 debug mirror 的 `sandbox.reveal.text`（既有 `visible/phase/wordKey/startedAt/finishedAt/rendered/blockedReason` 保持不變），讓 debug panel 與 `window.__CHAT_DEBUG__.sandbox.reveal` 都能直接檢視 reveal 文字 payload，避免僅有 `word.reveal.*` 時誤判 render 缺值。
- [sandbox][integration-fix][reveal-visible-render-guard] 修正答題後 `REVEAL_WORD` 只做 flow transition 未確保可見顯示：改由 authoritative `reveal` state（startedAt/finishedAt/rendered/blockedReason/text/base/rest）驅動 reveal，新增最短可見時長 `SANDBOX_REVEAL_VISIBLE_MIN_MS=2500`；僅在 reveal 完成且有 render 證據（或明確 missing_word_text）才允許 `reveal_word_done -> POST_REVEAL_CHAT`，避免 flow 誤報 done 但畫面未顯示。
- [sandbox][night-smoke-test][prompt-visibility-authoritative-sync] 將 `Run Full Night Test` 正名為 `Run Night Smoke Test`（按鈕/flow panel/audit key 全部一致），並修正題目切換可見性：題目轉換與 Force Next 都必須建立 `currentPrompt`、同步 `renderSync.stateQuestionId/renderedQuestionId`，在可作答步驟即使 scene 尚在切換也以 `scene_not_synced_warning` 保持 prompt 可見，避免 flow 已進到 `WAIT_REPLY_2` 但畫面無題目的 hidden state。
- [sandbox][full-night-test][authoritative-auto-submit-consume-path] 修正 `Run Full Night Test` Q1 自動答題改走與真人送出相同 submit path：建立同一 player message、帶入 messageId/sourceType/target/sourceMessageId 等 metadata 後進入同一 `consumePlayerReply` authoritative consume+parse+judge+gate path；成功必須同時滿足 parse/judge audit/consumedAt/flow 前進。失敗分類細分為 `message_injected_but_not_consumed`、`message_rejected_by_gate`、`parse_failed:*`、`judge_failed`，不再以聊天室顯示訊息視為成功。
- [sandbox][full-night-test][authoritative-second-question-judge-audit] 修正 Run Full Night Test 假失敗：第二題成功改用 authoritative 條件（flow>=WAIT_REPLY_2 / prompt.questionId==Q2 / replyGate consonant_answer+armed / nextQuestion emitted+toQuestionId==Q2）；render observer 延遲僅記 warning 不覆蓋成功。另補齊 judge audit 落盤 helper，保證 normal/auto/force-correct 都寫滿 parse+judge+source+consumedAt。
- [sandbox][integration-fix][q2-render-commit-authoritative] 修正「第二題 state 已切換但畫面未同步」：新增 authoritative `renderSync(stateQuestionId/renderedQuestionId/renderBlockedReason)`，將 `secondQuestionShown` 改為 `emit + prompt + rendered` 三者對齊才為 true；`ADVANCE_NEXT/Force Next` emit 第二題時強制依 `flow.questionIndex` 發出 scene/video 切換請求，避免舊場景/舊影片殘留。
- [sandbox][integration-fix][advance-next-post-reveal-ssot] 修正 `advance_next_effect` 與 post-reveal completion SSOT：`postRevealChatState` + `audit.transitions(post_reveal_chat_done)` + `backlogTechMessages.length===0` 收斂為單一完成來源，避免 `ADVANCE_NEXT` 在已完成狀態仍誤判 `post_reveal_chat_not_done`，並在 emit 後強制切換第二題 `currentPrompt` 與下一步 flow（Q1->`TAG_PLAYER_2_PRONOUNCE`、Q2->`TAG_PLAYER_3_MEANING`）。
- [sandbox][full-night-test][authoritative-submit-consume-failure-reason] 修正 `Run Full Night Test` 自動答題：送出後必須等待 authoritative consume + parse/judge（`flow.step` 脫離 `WAIT_REPLY_1` 或 gate consumed，且 `parse.raw` 有值、`parse.kind != not_evaluated`、`parse.ok=true`）；timeout 失敗改依 authoritative 停點回報 `answer_not_submitted`/`answer_not_consumed`/`judge_not_triggered`，避免誤報 `reveal_post_reveal`。
