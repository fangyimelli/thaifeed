- [sandbox][bootstrap hardening v2] `sandboxStoryMode.init()` 與 `getState()` 現在都帶 bootstrap guard，確保 mode mount 與 debug/poll read 都不會拿到未初始化 core state。
- [sandbox][trusted debug sync] 新增 `hydrateSandboxTrustedDebug()`；每次 runtime 啟動與 tick 都先把 `flow/scheduler/introGate/replyGate/prompt/lastReplyEval/audit.transitions` 從正式 mode state 回寫到 debug trusted 區。
