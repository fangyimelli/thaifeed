...
if (sceneViewSource.includes('useMemo')) {
  console.warn(`[netlify-build] Warning: token "useMemo" found in ${sceneViewPath}; continuing build.`);
} else {
...
if (appSource.includes('setChatTickRestartKey') || appSource.includes('chatTickRestartKey')) {
  console.warn(`[netlify-build] Warning: legacy chatTickRestartKey token found in ${appPath}; continuing build.`);
} else {
