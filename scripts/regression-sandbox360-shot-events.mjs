...
assertHas('<div ref={transformLayerRef} className="sandbox360TransformLayer" style={handheldTransformStyle}>', 'scene and overlays must share a single transform layer');
assertHas('const TV_ANCHOR = { x: 2256, y: 1054, w: 220, h: 118 } as const;', 'TV anchor must be a fixed authoritative scene-space constant');
assertHas('const tvAnchorRect: OverlayRect = {', 'TV anchor rect derivation missing');
assertHas('const tvDebugRect = useMemo(() => toScreenRect(overlaySceneRects.tv), [overlaySceneRects.tv, toScreenRect]);', 'TV debug rect mapping token missing');
assertHas('const tvOverlayRect = useMemo(() => toScreenRect(overlaySceneRects.tv), [overlaySceneRects.tv, toScreenRect]);', 'TV overlay rect mapping token missing');
assertHas('<div ref={tvDebugRef} className="sandbox360OverlayTvDebug" style={tvDebugRect}', 'TV debug overlay must share same authoritative anchor');
assertHas('<div ref={tvOverlayRef} className="sandbox360OverlayTvNoise" style={tvOverlayRect}', 'TV static overlay must share same authoritative anchor');
assertHas('<div>TV_ANCHOR: x={TV_ANCHOR.x}, y={TV_ANCHOR.y}, w={TV_ANCHOR.w}, h={TV_ANCHOR.h}</div>', 'debug panel must show TV_ANCHOR');
assertHas('<div>tvDebugRect: left={tvDebugRect.left}, top={tvDebugRect.top}, width={tvDebugRect.width}, height={tvDebugRect.height}</div>', 'debug panel must show tv debug rect');
assertHas('<div>tvOverlayRect: left={tvOverlayRect.left}, top={tvOverlayRect.top}, width={tvOverlayRect.width}, height={tvOverlayRect.height}</div>', 'debug panel must show tv overlay rect');
assertHas('<div>tv.sharedTransformContainer: {tvSharesTransformContainer ? \'true\' : \'false\'}</div>', 'debug panel must show shared transform container state');
assertHas("const transformLayerRef = useRef<HTMLDivElement | null>(null);", 'transform container ref missing');
assertHas("const tvDebugRef = useRef<HTMLDivElement | null>(null);", 'tv debug ref missing');
assertHas("const tvOverlayRef = useRef<HTMLDivElement | null>(null);", 'tv overlay ref missing');
assertHas("const [tvSharesTransformContainer, setTvSharesTransformContainer] = useState(false);", 'shared transform container observability state missing');
assertHas("const debugContainer = tvDebugEl?.closest('.sandbox360TransformLayer');", 'tv debug container lookup missing');
assertHas("const overlayContainer = tvOverlayEl?.closest('.sandbox360TransformLayer');", 'tv overlay container lookup missing');
